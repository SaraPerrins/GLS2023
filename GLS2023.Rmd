...
#   group_by(as.factor(state_tribal_1_state_2_tribal)) %>% 
#   mutate(PercentGatekeeper = prop.table(Any_Gatekeeper)*100) 
...
Table_Summaries <-  Table %>% 
  mutate(across(everything(), ~replace (., . == TRUE, 1)))  %>%
  dplyr::group_by(state_tribal_1_state_2_tribal) %>%
  # summarise(NumberGrantees = n()) %>%
  dplyr::mutate(across(c(Any_Gatekeeper:Any_LGBTQ),~ sum(.x)/n()*100), .groups = "keep") 
...
plot_loop_individual2 <- function (data = Table_AnnualReport, x=x, y = y) {{
  IndividualPlots2 <- ggplot(data = Table_AnnualReport, aes(y = .data[[y]], x = .data[[x]], fill = state_tribal_1_state_2_tribal)) + 
  ...
  png(paste0({y},"_individual.png"))
  # print(IndividualPlots)
  dev.off()
  print(IndividualPlots2)
  
}}

plotlist2 <-colnames(Table_AnnualReport)[-1] %>%
  map (~plot_loop_individual2(Table_AnnualReport, colnames(Table_AnnualReport)[1],.x))
